...
type ShopifyContact = { orderId: string; orderName: string; email: string; billingPhone: string; shippingName: string; firstName: string; lastName: string }
...
function ACAutomation() {
  const [section, setSection] = useState<'general' | 'supplements'>('general')
  ...
  const [supplementDate, setSupplementDate] = useState(today)
  const [supplementRows, setSupplementRows] = useState<Record<string, string>[]>([])
  ...
  const exportSupplementCsv = () => { const lines = [columns.map((column) => escapeCsv(column.label)).join(','), ...supplementRows.map((row) => columns.map((column) => escapeCsv(row[column.key] ?? '')).join(','))]; downloadFile(`\uFEFF${lines.join('\r\n')}`, 'text/csv;charset=utf-8', 'csv'); setExportOpen(false) }
  const exportSupplementExcel = () => { const cells = (values: string[], header = false) => values.map((value) => `<Cell${header ? ' ss:StyleID="Header"' : ''}><Data ss:Type="String">${escapeXml(value)}</Data></Cell>`).join(''); const sheetRows = `<Row>${cells(columns.map((column) => column.label), true)}</Row>${supplementRows.map((row) => `<Row>${cells(columns.map((column) => row[column.key] ?? ''))}</Row>`).join('')}`; downloadFile(`<?xml version="1.0"?><Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet" xmlns:ss="urn:schemas-microsoft-com:office:spreadsheet"><Styles><Style ss:ID="Header"><Font ss:Bold="1"/><Interior ss:Color="#E5F0DF" ss:Pattern="Solid"/></Style></Styles><Worksheet ss:Name="CS Supplements"><Table>${sheetRows}</Table></Worksheet></Workbook>`, 'application/vnd.ms-excel;charset=utf-8', 'xls'); setExportOpen(false) }

  const loadSupplementContacts = async () => {
    if (!supplementDate) return
    setSupplementLoading(true); setSupplementError('')
    try {
  ...
  return <main className="ac-sheet-shell ac-automation-layout">
    <aside className="ac-automation-sidebar" aria-label="AC automation sections">
      <span>Automation section</span>
      <button className={section === 'general' ? 'active' : ''} type="button" onClick={() => setSection('general')}>
        <b>CS</b><span>CS General<small>Current automation</small></span>
      </button>
      <button className={section === 'supplements' ? 'active' : ''} type="button" onClick={() => setSection('supplements')}>
        <b>CS</b><span>CS Supplements<small>Supplement contacts</small></span>
      </button>
    </aside>
    <section className="ac-automation-content">
    {section === 'general' ? <>
