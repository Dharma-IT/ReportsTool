...
type AcResponse = { contacts?: StripeContact[]; message?: string }
...
function ACAutomation() {
  ...
  const loadContacts = async () => {
    ...
    try {
      const response = await fetch(getApiUrl(`/api/ac-automation?from=${encodeURIComponent(from)}&to=${encodeURIComponent(to)}`))
      ...
      setRows((payload.contacts ?? []).map((contact, index) => ({
        email: contact.phone ? `${contact.phone}@dummy.com` : contact.email, firstName: contact.firstName, lastName: contact.lastName,
        preference: 'es', phone: contact.phone ? `+${contact.phone}` : '', treatment: 'Compounded Tirzepatide/B12/Gycine',
        source: 'Abandoned cart', owner: contactOwners[index % contactOwners.length], dealDate: displayDate(to),
      })))
  ...
  const connectStripe = async () => {
    setError(''); setLoginMessage('Opening Stripe login...')
    try {
      const response = await fetch(getApiUrl('/api/ac-automation?action=stripe-login'))
      const payload = await response.json() as AcResponse
      if (!response.ok) throw new Error(payload.message || 'Unable to open Stripe login.')
      setLoginMessage(payload.message || 'Complete login in the Stripe window.')
    } catch (caught) { setLoginMessage(''); setError(caught instanceof Error ? caught.message : 'Unable to open Stripe login.') }
  }
  ...
  return <main className="ac-sheet-shell">
    ...
    <section className="ac-sheet-card" aria-labelledby="ac-sheet-title">
      <div className="ac-sheet-toolbar"><div><span>ABANDONED CART DATABASE</span><h2 id="ac-sheet-title">Recovery contacts</h2></div><div className="ac-sheet-actions">
        <label className="ac-date-field">From <input type="date" value={from} max={to} onChange={(event) => setFrom(event.target.value)} /></label>
        <label className="ac-date-field">To <input type="date" value={to} min={from} onChange={(event) => setTo(event.target.value)} /></label>
        <button type="button" onClick={connectStripe}>Connect Stripe</button>
        <button type="button" onClick={loadContacts} disabled={loading || !from || !to}>{loading ? 'Loading...' : 'Apply'}</button>
      ...
      {loginMessage ? <div className="ac-sheet-notice">{loginMessage}</div> : null}
      ...
      <div className="ac-spreadsheet-wrap"><table className="ac-spreadsheet" style={{ minWidth: columns.reduce((sum, column) => sum + column.width, 42) }}><colgroup><col style={{ width: 42 }} />{columns.map((column) => <col key={column.key} style={{ width: column.width }} />)}</colgroup><thead><tr className="ac-letter-row"><th />{columns.map((column, index) => <th key={column.key}>{letters[index]}</th>)}</tr><tr className="ac-heading-row"><th>1</th>{columns.map((column) => <th key={column.key}><span>{column.label}</span><button type="button" onClick={() => setFiltering(true)}><FilterIcon /></button></th>)}</tr></thead><tbody>{Array.from({ length: visibleRowCount }, (_, index) => { const row = filteredRows[index]; return <tr key={index}><th>{index + 2}</th>{columns.map((column) => <td key={column.key}>{row?.[column.key] ?? (!row && column.dropdown ? <span className="ac-dropdown-cell"><i /></span> : '')}</td>)}</tr> })}</tbody></table>
        {!loading && !filteredRows.length ? <div className="ac-sheet-empty"><strong>{query ? 'No matching contacts' : 'No contacts'}</strong><p>{query ? `No contacts match “${query}”.` : error ? 'Correct the issue above and apply the date filter again.' : 'Choose an EST date range and click Apply.'}</p></div> : null}
